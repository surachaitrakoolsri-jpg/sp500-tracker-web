        ...
        <div className="spx-num spx-marquee-track" style={{ whiteSpace: "nowrap", fontSize: 13 }}>
          {[...Array(2)].map((_, rep) => (
            <span key={rep}>
              ...
              <span style={{ marginRight: 48 }}>เงินลงทุนเริ่มต้น {fmt(initial, currency)}</span>
              <span style={{ marginRight: 48 }}>DCA รายเดือน {fmt(monthly, currency)}</span>
              <span style={{ marginRight: 48 }}>มูลค่าโครงการปีที่ {years} : {fmt(projectedValue, currency)}</span>
              <span style={{ marginRight: 48 }}>พอร์ตผสมคาดการณ์ {pct(blendedReturn)} ต่อปี</span>
              <span style={{ marginRight: 48 }}>Max drawdown จำลอง {pct(maxDrawdown)}</span>
            </span>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 780, margin: "0 auto", padding: "32px 24px 0" }}>
        ...
        {tab === "overview" && (
          <>
            ...
            <div style={{
              background: COLORS.forest, color: COLORS.parchment, borderRadius: 4, padding: "24px 28px",
              display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 20, marginBottom: 24,
            }}>
              <div>
              ...
              <div style={{ display: "flex", gap: 28 }}>
                <div>
                  <div style={{ fontSize: 11, color: COLORS.goldLight, textTransform: "uppercase", letterSpacing: "0.06em" }}>เงินลงทุนสะสม</div>
                  <div className="spx-num" style={{ fontSize: 18 }}>{fmt(totalContributed, currency)}</div>
                </div>
                <div>
        ...
        {tab === "drawdown" && (
          <div className="spx-card">
            ...
            <div style={{ display: "flex", gap: 24, marginBottom: 16 }}>
              <div>
                <div className="spx-label">Max drawdown จำลอง</div>
                <div className="spx-num" style={{ fontSize: 22, color: COLORS.red, fontWeight: 600 }}>{pct(maxDrawdown)}</div>
              </div>
              <div>
                <div className="spx-label">ผลตอบแทน / ความผันผวนที่ใช้</div>
                <div className="spx-num" style={{ fontSize: 15 }}>{pct(blendedReturn)} / {pct(blendedVol)}</div>
              </div>
            </div>
        ...
        {tab === "import" && (
          <div className="spx-card">
            ...
            <div style={{ background: COLORS.paper, border: `1px dashed ${COLORS.rule}`, borderRadius: 4, padding: "10px 14px", marginBottom: 16 }}>
